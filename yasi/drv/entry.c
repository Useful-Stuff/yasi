...
VOID KernelProtect_Unload(PDRIVER_OBJECT  DriverObject);    
...
#pragma alloc_text(PAGE, KernelProtect_Unload)
...
NTSTATUS DriverEntry(PDRIVER_OBJECT  pDriverObject, PUNICODE_STRING  pRegistryPath)
{
    ...
    if(NtStatus == STATUS_SUCCESS)
    {
        ...
        for(uiIndex = 0; uiIndex < IRP_MJ_MAXIMUM_FUNCTION; uiIndex++)
             pDriverObject->MajorFunction[uiIndex] = KernelProtect_UnSupportedFunction;
    
        pDriverObject->MajorFunction[IRP_MJ_CLOSE]             = KernelProtect_Close;
        pDriverObject->MajorFunction[IRP_MJ_CREATE]            = KernelProtect_Create;
        pDriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL]    = KernelProtect_IoControl;
        pDriverObject->MajorFunction[IRP_MJ_READ]              = KernelProtect_Read;
...
{    
    ...
    DbgPrint("KernelProtect_Unload Called \r\n");
